...
fn avr_savecontext(presave_code: [*:0]const u8, load_address_to_Z_code: [*:0]const u8) callconv(.Naked) noreturn {
    asm volatile (
        \\; push Zdd 
        \\push r30
        \\push r31
...
fn avr_restorecontext(load_address_to_Z_code: [*:0]const u8) callconv(.Naked) noreturn {
    asm volatile (
...
export fn avr_save_context_global_pointer(presave_code: [*:0]const u8, global_context_pointer: [*:0]const u8) callconv(.Naked) noreturn {
...
export fn avr_restore_context_global_pointer(global_context_pointer: [*:0]const u8) callconv(.Naked) noreturn {
    avr_restorecontext(
        \\lds ZL, 
    ++ global_context_pointer ++
        \\\n
...
export fn avr_getcontext(cp: *ContextAVR) callconv(.Naked) void {
    _ = cp;
    comptime {
...
export fn avr_setcontext(cp: *ContextAVR) callconv(.Naked) void {
    _ = cp;
    comptime {
...
export fn avr_swapcontext(oucp: *ContextAVR, ucp: *ContextAVR) callconv(.Naked) void {
    _ = oucp;
    _ = ucp;
    comptime {
        avr_savecontext("", "mov r30, r24\nmov r31, r25\n");
...
export fn avr_makecontext_callfunc(successor: *ContextAVR, func: ContextFuncParam, args: *void) void {
    const func_call: *const fn (argp: *void) void = @ptrCast(func);
    func_call(args);
    avr_setcontext(successor);
}

export fn avr_makecontext(cp: *ContextAVR, stackp: *void, stack_size: usize, successor_cp: *ContextAVR, funcp: ContextFuncParam, funcargs: *void) callconv(.Naked) noreturn {
